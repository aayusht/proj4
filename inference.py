...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for varElim in eliminationOrder:
            others, joined = joinFactorsByVariable(factors_in, varElim)
            factors_in = []

            if len(joined.unconditionedVariables()) > 1:
                elim = eliminate(joined, varElim)
                factors_in.append(elim)
            factors_in += others

        final = joinFactors(factors_in)
